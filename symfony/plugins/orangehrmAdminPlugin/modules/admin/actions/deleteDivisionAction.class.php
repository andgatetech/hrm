...
class deleteDivisionAction extends sfAction {
    
    private $divisionService;
    
    public function getDivisionService() {
        
        if (!($this->divisionService instanceof DivisionService)) {
    ...
    public function execute($request) {
        ...
        if (!empty($toDeleteIds) && $request->isMethod('post')) {
            ...
            if ($form->isValid()) {
                $result = $this->getDivisionService()->deleteDivisions($toDeleteIds);
            }
                if ($result) {
